...
#     derived from the latest persisted OCRValidationResult for that document.
#     This file-level shape is required by the Correct/Incorrect filters and
#     allows Select All to operate on individual invoices in a batch.
#     """
...
#             limit = min(max(1, int(request.query_params.get("limit", 100))), 100)
...
#         document_ids = [upload.document_id for upload in uploads if upload.document_id]
#         latest_validation = {}
...
#                 if validation.document_id not in latest_validation:
#                     latest_validation[validation.document_id] = validation

#         results = []

#         for upload in uploads:
#             document = getattr(upload, "document", None)
#             validation = latest_validation.get(upload.document_id)

#             results.append(
#                 {
#                     "type": "single",
#                     "batch_id": str(upload.batch_id) if upload.batch_id else None,
#                     "document_id": str(document.id) if document else None,
#                     "upload_id": str(upload.id),
#                     "filename": upload.original_filename,
#                     "file_count": 1,
#                     "status": upload.status,
#                     "source_type": getattr(upload.batch, "source_type", None),
...
#                     "validation_updated_at": validation.created_at if validation else None,
#                 }
#             )

#         total = len(results)
#         page_results = results[offset : offset + limit]
#         serializer = OCRHistoryEntrySerializer(page_results, many=True)
...
#         allowed_history_statuses = {value for value, _label in OCRUpload.Status.choices}
#         if status_filter and status_filter not in allowed_history_statuses:
#             return Response(
#                 {"detail": "Invalid OCR history status filter."},
#                 status=status.HTTP_400_BAD_REQUEST,
#             )

#         uploads_queryset = OCRUpload.objects.filter(
#             batch__in=_visible_batch_queryset(request.user)
#         )
#         if status_filter:
#             uploads_queryset = uploads_queryset.filter(status=status_filter)
